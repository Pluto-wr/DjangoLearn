...
def like_change(request):
    ...
    try:
        ...
        return ErrorResponse(401, 'object ont exist')
    # 处理数据
    if request.GET.get('is_like') == 'true':
        # 要点赞的情况
        like_record, created = LikeRecord.objects.get_or_create(content_type=content_type, object_id=object_id, user=user)   # 获取不到就创建
        if created:
            # 未点赞过,进行点赞
